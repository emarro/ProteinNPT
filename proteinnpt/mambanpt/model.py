...
class MambaNPTModel(nn.Module):
    def __init__(self, args, alphabet):
        ...
        self.target_names = [
            x
            for x in self.args.target_config.keys()
            if self.args.target_config[x]["in_NPT_loss"]
        ]
        self.num_targets_input = len(
            self.target_names_input
        )  # Includes all targets, incl. zero-shot fitness predictions
        self.num_targets = len(
            self.target_names
        )  # Number of actual targets we want to predict
        ...
        if (
            ...
            config = json.load(
                open(args.embedding_model_location + os.sep + "config.json")
            )
            ...
            self.aa_embedding = TranceptionLMHeadModel.from_pretrained(
                pretrained_model_name_or_path=args.embedding_model_location,
                config=config,
            )
        ...
        if self.aa_embedding_dim != self.args.embed_dim:  # Need to project internally
            ...
            self.token_embedding_expansion = nn.Linear(
                self.args.embed_dim, self.aa_embedding_dim
            )

        self.target_embedding = nn.ModuleDict(
            {
                target_name: nn.Linear(
                    self.args.target_config[target_name]["dim"]
                    + 1,  # Need to add one as we append the mask flag to each input target
                    self.args.embed_dim,
                )
                if self.args.target_config[target_name]["type"] == "continuous"
                else nn.Embedding(
                    self.args.target_config[target_name]["dim"]
                    + 1,  # Size of the dictionary of embeddings. Need to add 1 for the mask flag as well
                    self.args.embed_dim,
                )
        ...
        #            getattr(
        #                self.args, "max_tokens_per_msa", self.args.max_tokens_per_msa
        #            ),
        ...
        if args.target_prediction_model == "CNN":
            self.layer_pre_head = nn.Sequential(
                nn.Conv1d(
                    in_channels=target_pred_input_dim,
                    out_channels=target_pred_input_dim,
        ...
        if self.args.target_prediction_head == "Target_embeddings_only":
            ...
            self.args.target_prediction_head
            == "Target_embeddings_and_AA_embeddings_mean_pooled"
        ):
        ...
        if self.args.augmentation == "zero_shot_fitness_predictions_covariate":
            self.zero_shot_fitness_prediction_weight = nn.ModuleDict(
                {
                    target_name: nn.Linear(
                        1, self.args.target_config[target_name]["dim"], bias=False
                    )
                    for target_name in self.target_names
                }
            )
            for target_name in self.target_names:
                torch.nn.init.constant_(
                    self.zero_shot_fitness_prediction_weight[target_name].weight, 1e-4
                )

        self.target_pred_head = nn.ModuleDict(
            {
                target_name: nn.Linear(
                    target_pred_input_dim, self.args.target_config[target_name]["dim"]
                )
                for target_name in self.target_names
            }
        )
    ...
    def forward(
        ...
        if (
            self.args.aa_embeddings == "MSA_Transformer"
            and self.args.sequence_embeddings_location is None
        ):  # If loading MSAT embeddings from disk, we have dropped one dim already
            assert tokens.ndim == 3, "Finding dimension of tokens to be: {}".format(
                tokens.ndim
            )
            (
                num_MSAs_in_batch,
                num_sequences_in_alignments,
                seqlen,
            ) = tokens.size()  # N, B, L (seqs with labels, seqs in MSA, seq length)
            batch_size = num_MSAs_in_batch
        else:
            assert tokens.ndim == 2, "Finding dimension of tokens to be: {}".format(
                tokens.ndim
            )
        ...
        if sequence_embeddings is not None:
            ...
            if self.args.aa_embeddings == "MSA_Transformer":
                ...
                x = x + self.aa_positions_embedding(
                    tokens.view(batch_size, seqlen)
                ).view(
                    x.size()
                )  # Need position embedding in PNPT since we will apply axial attention
            else:
                print("AA embeddings not recognized")
                sys.exit(0)

        if self.aa_embedding_dim != self.args.embed_dim:
            x = self.token_embedding_projection(x)

        if (
            self.args.target_prediction_head
            != "Target_embeddings_and_AA_embeddings_mean_pooled"
        ):  # We mix AA embeddings pre NPT
        ...
        for target_name in self.target_names_input:
            if self.args.target_config[target_name]["type"] == "continuous":
                num_sequences_with_target, dim_targets = targets[
                    target_name
                ].shape  # N, D_t #In most cases dim_targets = D_t = 2 (original dimension of continuous input + 1 dim for mask)
            else:
                num_sequences_with_target = targets[
                    target_name
                ].shape[
                    0
                ]  # Input categorical targets is unidmensional ie a vector of category indices
                targets[target_name] = targets[
                    target_name
                ].long()  # Ensure we cast to integer before passing to Embedding layer for categorical targets
            y.append(
                self.target_embedding[target_name](targets[target_name]).view(
                    num_sequences_with_target, 1, self.args.embed_dim
                )
            )
        y = torch.cat(
            y, dim=-2
        )  # concatenate across second to last dimension # N, num_targets, D
        assert y.shape == (
            num_sequences_with_target,
            self.num_targets_input,
            self.args.embed_dim,
        ), "Error in y shape: {}".format(y.shape)
        y = y.view(
            1, num_sequences_with_target, self.num_targets_input, self.args.embed_dim
        )  # 1, N, num_targets, D
        ...
        if self.aa_embedding_dim != self.args.embed_dim:
            logits_protein_sequence = self.lm_head(
                self.token_embedding_expansion(x[..., :seqlen, :])
            )
        else:
            logits_protein_sequence = self.lm_head(
                x[..., :seqlen, :]
            )  # Remove dependency on targets for final AA predictions. logits size: (1, N, L, Vocab)
        ...
        if (
            ...
            x = x.mean(dim=-2)  # N, D
            y = y.view(
                num_sequences_with_target, self.num_targets_input * self.args.embed_dim
            )
        ...
        for target_index, target_name in enumerate(self.target_names):
            if (
                ...
                target_predictions[target_name] = (
                    self.target_pred_head[target_name](y)
                    .view(-1, self.args.target_config[target_name]["dim"])
                    .squeeze(dim=-1)
                )  # We use the concatenated X and target embeddings (all of them) to predict each target
            else:
                if self.args.target_prediction_model == "MLP":
                    y[:, target_index, :] = self.layer_pre_head[target_name](
                        y[:, target_index, :]
                    )
                target_predictions[target_name] = (
                    self.target_pred_head[target_name](y[:, target_index, :])
                    .view(-1, self.args.target_config[target_name]["dim"])
                    .squeeze(dim=-1)
                )  # input the embedding with the relevant target_index
            if self.args.augmentation == "zero_shot_fitness_predictions_covariate":
                target_predictions[target_name] += (
                    self.zero_shot_fitness_prediction_weight[target_name](
        ...
        result = {
            "logits_protein_sequence": logits_protein_sequence,
    ...
    def forward_with_uncertainty(
        ...
        zero_shot_fitness_predictions=None,
        sequence_embeddings=None,
        num_MC_dropout_samples=10,
        number_of_mutated_seqs_to_score=None,
    ):
        """
        ...
        with torch.no_grad():
            ...
            for _ in range(num_MC_dropout_samples):
                target_predictions_sample = self.forward(
                    tokens,
                    targets,
                ...
                for target_name in self.target_names:
                    predictions_dict[target_name].append(
                        target_predictions_sample[target_name]
                    )
            results_with_uncertainty = {}
            for target_name in self.target_names:
                concatenated_target_pred = torch.cat(
                    [x.view(-1, 1) for x in predictions_dict[target_name]], dim=-1
                )
                results_with_uncertainty[target_name] = {}
                results_with_uncertainty[target_name][
                    "predictions_avg"
                ] = concatenated_target_pred.mean(dim=-1)
                results_with_uncertainty[target_name][
                    "uncertainty"
                ] = concatenated_target_pred.std(dim=-1)
    ...
    def protein_npt_loss(
        self,
        token_predictions_logits,
        token_labels,
        target_predictions,
        target_labels,
        MLM_reconstruction_loss_weight,
        label_smoothing=0.0,
    ):
        ...
        if (token_labels is not None) and (MLM_reconstruction_loss_weight > 0.0):
            if (
                ...
                token_labels = token_labels[
                    :, 0, :
                ]  # Only keep the token labels for seq to score. Drops the token labels for MSA sequences
            reconstruction_loss = CrossEntropyLoss(
                reduction="mean", label_smoothing=label_smoothing
            )(
                token_predictions_logits.reshape(-1, self.alphabet_size),
        ...
        for target_name in self.target_names:
            if self.args.target_config[target_name]["in_NPT_loss"]:
                loss_masked_targets = ~target_labels[target_name].eq(
                    -100
                )  # Masked items are the ones for which the label was not set to -100
                if (
                    loss_masked_targets.sum() == 0
                    or torch.isnan(
                        target_labels[target_name][loss_masked_targets]
                    ).sum()
                    > 0
                ):  # First condition true if we dont mask anything (eg., all target missing at eval). Second condition true if we force-mask one value at train time (to satisfy min_num_labels_masked in mast_target()) and corresponding target value is missing
                    tgt_loss = torch.tensor(0.0)
                else:
                    if self.args.target_config[target_name]["type"] == "continuous":
                        tgt_loss = MSELoss(reduction="mean")(
                            target_predictions[target_name][loss_masked_targets],
                            target_labels[target_name][loss_masked_targets],
                        )
                    else:
                        tgt_loss = CrossEntropyLoss(
                            reduction="mean", label_smoothing=label_smoothing
                        )(
                            target_predictions[target_name][loss_masked_targets].view(
                                -1, self.args.target_config[target_name]["dim"]
                            ),
                            target_labels[target_name][loss_masked_targets]
                            .view(-1)
                            .long(),
                        )  # Note: we dont add one to the # of categories in the CE loss here (we dont predict <mask>)
                ...
                total_loss += (
                    target_prediction_loss_weight * target_prediction_loss[target_name]
                )
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            decay_parameters = [
                ...
                if (
                    "bias" not in name
                    and "pseudo_likelihood_weight" not in name
                    and "zero_shot_fitness_prediction_weight" not in name
                )
            ]
            psl_decay_parameters = [
                name
                for name in all_parameters
                if (
                    "bias" not in name
                    and (
                        "pseudo_likelihood_weight" in name
                        or "zero_shot_fitness_prediction_weight" in name
                    )
                )
            ]
            optimizer_grouped_parameters = [
                ...
                },
                {
                    "params": [
                        p
                        for n, p in self.named_parameters()
                        if (n not in decay_parameters and n not in psl_decay_parameters)
                    ],
                    "weight_decay": 0.0,
                },
            ]
